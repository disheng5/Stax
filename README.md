...
3. 当前 `miniprogram/app.js` 已配置真实云环境；导入该 AppID 对应的项目后可直接连接 CloudBase
4. 部署到其他小程序时，将 `miniprogram/app.js` 的 `ENV_ID` 替换为目标云环境 ID，并按 `docs/CLOUD_SETUP.md` 创建 7 个集合、索引和 19 个云函数
5. 如需纯本地 UI 演示，将 `ENV_ID` 临时设为 `STAX_REPLACE_ME_BEFORE_DEPLOY`，应用会启用内存 Mock
